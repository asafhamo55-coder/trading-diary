...
import { DEMO_TRADES, DEMO_ACCOUNT, DEMO_MONTHLY_REVIEWS } from "@/lib/demo-data";
...
export default function PortfolioPage() {
  const portfolio = useMemo(() => {
    ...
    DEMO_MONTHLY_REVIEWS.forEach((r) => {
      if (r.riskUnit !== null) riskUnits.set(r.month, r.riskUnit);
    });

    return calculatePortfolioTracker(tradesByMonth, DEMO_ACCOUNT.startingBalance, riskUnits);
  }, []);

  const totalPnL = portfolio.reduce((sum, m) => sum + m.monthlyPnL, 0);
  const totalTrades = portfolio.reduce((sum, m) => sum + m.tradeCount, 0);
  const finalBalance = portfolio[portfolio.length - 1]?.portfolioEnd ?? DEMO_ACCOUNT.startingBalance;
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-2 rounded-xl bg-[#151921] border border-[#2A3040] px-4 py-2">
          ...
          <span className="text-sm font-bold text-[#E8ECF4]">
            {formatCurrency(DEMO_ACCOUNT.startingBalance)}
          </span>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tfoot>
              <tr className="bg-[#1C2130] font-semibold">
                ...
                >
                  {totalPnL >= 0 ? "+" : ""}
                  {formatPercent(totalPnL / DEMO_ACCOUNT.startingBalance)}
                </td>
              </tr>
            </tfoot>
          </table>
