...
import { DEMO_MONTHLY_REVIEWS } from "@/lib/demo-data";
...
function getFormForMonth(month: number): ReviewForm {
  const review = DEMO_MONTHLY_REVIEWS.find((r) => r.month === month);
  if (!review) {
    return {
      goal1: "",
      goal2: "",
...
export default function JournalPage() {
  ...
  const [form, setForm] = useState<ReviewForm>(() => getFormForMonth(1));

  function handleMonthChange(month: number) {
    setSelectedMonth(month);
    setForm(getFormForMonth(month));
  }
